---
title: eth_getFilterChanges | zkEVM
openapi: /openapi/polygon_zkevm_node_api/eth_getFilterChanges.json POST /942aad90bb6a082676497030b81e40ba
---

Polygon zkEVM API polling method used to retrieve updates from a filter. A filter is an object used to track changes to the state of the blockchain.
...
The `eth_getFilterChanges` method can poll a filter to periodically retrieve new data. You can, for example, create a blocks filter using [eth\_newBlockFilter](/reference/zkevm-newblockfilter) and periodically poll it to get new blocks.
