---
title: eth_sendRawTransaction | zkEVM
openapi: /openapi/polygon_zkevm_node_api/eth_sendRawTransaction.json POST /942aad90bb6a082676497030b81e40ba
---

Polygon zkEVM API method that allows submitting a signed transaction to the network. Once a transaction is signed, you can use the `eth_sendRawTransaction` method to submit the signed transaction to the Ethereum network for processing.
...
One potential use case for `eth_sendRawTransaction` is to automatically swap ETH for a token when its price reaches a certain level. `eth_sendRawTransaction` will allow custom transaction parameters, such as gas price or gas limit, to be specified to optimize the speed and cost of the transaction.
