...
<CodeGroup>
  ...
  async function handleNewPending(transactionHash) {
      try {
          ...
          if (value >= 100000000000000000000) { // 100 MATIC in Wei
              console.log(`----- New pending transaction ------`);
              console.log(`From: ${from}`)
              console.log(`To: ${to}`)
              console.log(`Value: ${Number(web3.utils.fromWei(value, 'ether')).toFixed(2)} MATIC \n`)
          }
      } catch (error) {
