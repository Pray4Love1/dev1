...
description: Use eth_clearSubscriptions on Arbitrum via web3.js to remove all active WebSocket subscriptions from your Chainstack node connection at once.
---
...
`boolean` — keep the [subscribe("syncing")](/reference/arbitrum-subscribesyncing) subscription if `true`.
...
The following code is an example using the [subscribe("newBlockHeaders")](/reference/arbitrum-subscribenewblockheaders) method, which will be stopped and removed after one minute using `clearSubscriptions`:
