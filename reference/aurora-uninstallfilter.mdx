---
title: eth_uninstallFilter | Aurora
openapi: /openapi/aurora_node_api/eth_uninstallFilter.json POST /6df1a1b3061097e66349993a96b8e535
---

Aurora API method used to remove a filter previously created using one of the following methods:
...
One use case for `eth_uninstallFilter` is to optimize resource usage in a DApp. When a DApp needs to monitor events on the blockchain, it can create a filter using [eth\_newFilter](/reference/aurora-newfilter) or [eth\_newBlockFilter](/reference/aurora-newblockfilter) to listen for specific events or blocks.
