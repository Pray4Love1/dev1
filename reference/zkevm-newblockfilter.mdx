---
title: eth_newBlockFilter | zkEVM
openapi: /openapi/polygon_zkevm_node_api/eth_newBlockFilter.json POST /942aad90bb6a082676497030b81e40ba
---

Polygon zkEVM API method that creates a filter that watches for new blocks on the blockchain. It returns a filter ID, which can be used to retrieve the results using the [eth\_getFilterChanges](/reference/zkevm-getfilterchanges) method. The `eth_newBlockFilter` method is useful for developers who must be notified of new blocks on the blockchain in real-time.
