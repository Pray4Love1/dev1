---
title: 'subscribe ("logs") | Arbitrum'
---

web3.js subscription equivalent to [eth\_newFilter](/reference/arbitrum-newfilter). `subscribe("logs")` allows developers to subscribe to real-time updates about new event logs on the Arbitrum blockchain; the application will receive notifications whenever new logs matching the filter are emitted.
...
The following is an implementation of this concept using web3.js subscriptions, this program will only track the `Transfer` event from the [Arbitrum Odyssey NFT](https://arbiscan.io/token/0xfAe39eC09730CA0F14262A636D2d7C5539353752#code) smart contract.

The `PunkTransfer` looks like this in the smart contract:
...
<CodeGroup>
  ...
  async function handleLogs(logs) {
      ...
      console.log(`An Arbitrum NFT was transferred in this block: ${block}`)
  }
