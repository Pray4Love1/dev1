---
title: "About Quorum"
description: Learn about Quorum, a Go Ethereum fork for permissioned consortium blockchains supporting Raft and IBFT consensus. Now deprecated on Chainstack.
---
...
* Raft — a CFT consensus algorithm.
* Istanbul Byzantine Fault Tolerance — a BFT consensus algorithm.

Raft can tolerate an **f** number of faulty nodes in **n=2f+1**, where **n** is the total number of nodes. IBFT can tolerate an **f** number of faulty nodes in **n=3f+1**, where **n** is the total number of nodes.
...
* Explorer — a node running [Sirato explorer](https://github.com/web3labs/sirato-free) of a developer edition.

  <Warning>
    ### Developer edition of explorer

    Sirato provides a developer edition of the explorer for evaluation purposes. It has a limited number of features, for example, you cannot upload smart contracts metadata through the Chainstack nodes.

    If you are looking for a wider [range of features](https://docs.sirato.io/features/navigation), consider using one of the [hosted explorer options](https://www.web3labs.com/referrals/chainstack).
  </Warning>
