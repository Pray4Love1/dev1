...
description: Build a trustless token-swap escrow on Solana with Anchor 1.0.0 — PDA-owned vaults, the token interface, CPI from a PDA signer, and atomic maker/taker exchange with refund.
---
...
<Warning>
`init-if-needed` is gated behind a feature flag because it can mask bugs (a typo in constraints will silently initialize a fresh account instead of erroring). We use it deliberately in `take` and `refund` so the caller does not have to pre-create recipient token accounts. Always pair `init_if_needed` with explicit mint/authority constraints so Anchor validates the account even when it already exists.
</Warning>
...
- **Transfer hooks** — fire a CPI into a third-party program on every transfer. Your `transfer_checked` call now passes through arbitrary code. Whitelist trusted hook programs or reject mints with transfer hooks entirely.
