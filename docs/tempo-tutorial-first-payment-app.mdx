...
description: Send TIP-20 stablecoin transfers with payment memos on the Tempo blockchain using Foundry and a Chainstack RPC endpoint for real-time settlement.
---
...
Replace `YOUR_CHAINSTACK_ENDPOINT` with your actual Chainstack Tempo node endpoint:

```javascript
const { ethers } = require("ethers");

// Configuration - use your Chainstack endpoint
