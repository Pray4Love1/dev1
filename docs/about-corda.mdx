---
title: "About Corda"
description: Learn about Corda, an open-source permissioned blockchain platform for private transactions between identifiable parties. Now deprecated on Chainstack.
---
...
* Single Notary — one notary node runs the [notary service](/docs/about-corda#notary-service) for the deployed network.
* Raft — a CFT consensus implementation on [Atomix](https://atomix.io/). See [Crash fault-tolerant (experimental)](https://docs.corda.net/running-a-notary.html#crash-fault-tolerant-experimental).
* BFT-SMaRt — a BFT consensus implementation. See [Byzantine fault-tolerant (experimental)](https://docs.corda.net/running-a-notary.html#byzantine-fault-tolerant-experimental).
...
* A node with a [notary service](/docs/about-corda#notary-service) — a notary node

### Network map service

The network map service is the catalog of all nodes on a [compatibility zone](/docs/about-corda#compatibility-zones) with the information on the node identities, node certificates, and node IP addresses.
...
* Contract — a set of rules for a transaction to be considered valid and accepted. See [Contracts](/docs/about-corda#contracts).
* Flow — an automated set of actions for the node to run on receiving an RPC request from the node owner. See [Flows](/docs/about-corda#flows).
...
* The transaction does not have a double-spend as verified by the [notary service](/docs/about-corda#notary-service).
* The transaction meets the [contract](/docs/about-corda#contracts) rules as part of a CorDapp running on all nodes that are a part of the transaction.
